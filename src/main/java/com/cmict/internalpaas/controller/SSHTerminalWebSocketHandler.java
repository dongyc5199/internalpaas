...
public class SSHTerminalWebSocketHandler implements WebSocketHandler {
    ...
    private void handleTextMessage(WebSocketSession session, TextMessage message) {
        logger.info("收到WebSocket消息: {}", message.getPayload()); // 添加这行
        try {
            JsonNode jsonNode = objectMapper.readTree(message.getPayload());
            String type = jsonNode.get("type").asText();
            logger.info("消息类型: {}", type); // 添加这行
            switch (type) {
                case "connect":
    ...
    private void handleConnect(WebSocketSession session, JsonNode jsonNode) {
        try {
            ...
            String terminalType = jsonNode.has("terminalType") ? jsonNode.get("terminalType").asText()
                    : "xterm-256color";
    ...
    private void handleInput(WebSocketSession session, JsonNode jsonNode) {
        try {
            ...
            logger.info("WebSocket会话ID: {}", session.getId());
            logger.info("映射的SSH会话ID: {}", sshSessionId);
            logger.info("当前活跃映射数量: {}", webSocketToSSHMapping.size());
            logger.info("输入内容: [{}]", input);

            if (sshSessionId != null) {
                logger.info("开始调用sshTerminalService.sendCommand...");
                try {
                    sshTerminalService.sendCommand(sshSessionId, input);
                    logger.info("sshTerminalService.sendCommand 调用完成");
                } catch (Exception e) {
                    logger.error("调用sshTerminalService.sendCommand失败", e);
                    throw e; // 重新抛出异常以便外层catch处理
                }
            } else {
                logger.error("SSH会话ID为null，WebSocket会话: {}", session.getId());
                logger.info("当前所有WebSocket映射: {}", webSocketToSSHMapping);

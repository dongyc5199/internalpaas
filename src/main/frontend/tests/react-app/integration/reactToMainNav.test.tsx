...
import type { NavigationEventDetail } from "../../../react-app/types/navigation";
...
describe("Integration Test: React to Main App Navigation Update", () => {
    ...
    let capturedEvents: Array<CustomEvent<NavigationEventDetail>>;
    let removeReactNavListener: (() => void) | undefined;
    ...
    beforeEach(() => {
        ...
        const captureReactNavEvent = (event: Event): void => {
            if (event instanceof CustomEvent && event.type === "react-nav-change") {
                const detailEvent = event as CustomEvent<NavigationEventDetail>;

                if (detailEvent.detail) {
                    capturedEvents.push(detailEvent);
                }
            }
        };

        window.addEventListener("react-nav-change", captureReactNavEvent as EventListener);
        removeReactNavListener = (): void => {
            window.removeEventListener("react-nav-change", captureReactNavEvent as EventListener);
        };
    });
    ...
    afterEach(() => {
        ...
        removeReactNavListener?.();
        removeReactNavListener = undefined;
        capturedEvents = [];
        vi.restoreAllMocks();
    });
    ...
    describe("Sequential Route Changes", () => {
        ...
        it("should dispatch events with increasing timestamps", async () => {
            ...
            const timestamps = capturedEvents
                .map((e) => e.detail?.timestamp)
                .filter((value): value is number => typeof value === "number");

            expect(timestamps).toHaveLength(capturedEvents.length);
    ...
    describe("Anti-Loop Mechanism", () => {
        it("should not dispatch event when navigation is triggered by main app", async () => {
            ...
            // 模拟React Router根据主应用事件同步到 /releases
            ...
            act(() => {
                rerender({ location: syncedLocation });
            });

            // 等待防循环窗口内仍不会抛出react-nav-change事件
            await act(async () => {
                await new Promise((resolve) => setTimeout(resolve, 200));
            });

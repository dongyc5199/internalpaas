...
export function useNavSync(navigate: NavigateFunction, location: Location): void {
    ...
    useEffect(() => {
        if (!isEmbedded) {
            if (window.__DEPLOY_PLATFORM_DEBUG__) {
                console.log(
                    "[useNavSync] Standalone mode detected, skipping main-nav-change listener"
                );
            }
        ...
        const handleMainNavChange = (event: Event): void => {
            ...
            const currentPathname = prevPathnameRef.current;
        ...
        return (): void => {
            ...
            if (isSyncingRef.current) {
                ...
                if (window.__DEPLOY_PLATFORM_DEBUG__) {
                    console.log("[useNavSync] Sync flag reset during cleanup");
                }
            }
    ...
    }, [navigate, isEmbedded]); // 依赖项: navigate以及嵌入模式
    ...
    useEffect(() => {
        ...
        if (!isEmbedded) {
            ...
            if (window.__DEPLOY_PLATFORM_DEBUG__) {
                console.log(
                    "[useNavSync] Standalone mode detected, skipping react-nav-change dispatch"
                );
            }
        ...
        if (isSyncingRef.current) {
            if (window.__DEPLOY_PLATFORM_DEBUG__) {
                console.log(
                    "[useNavSync] Skipping react-nav-change dispatch (syncing in progress)"
                );
            }
        ...
        // 更新prev pathname
        prevPathnameRef.current = currentPathname;
    }, [location.pathname, isEmbedded]); // 依赖项: pathname变化和嵌入模式
}
